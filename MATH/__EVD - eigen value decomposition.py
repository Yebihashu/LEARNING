...
> The variance information is not only per feature, because it doesn't give enough information about the dat abehaviour. 
  ...
  where
  V(x,x) = E((x-E(x))^2)         = (x-E(x)) * (x-E(x)).T / N 
  V(y,y) = E((y-E(y))^2)         = (y-E(y)) * (y-E(y)).T / N 
  V(x,y) = E((x-E(x))*(y-E(y)))  = (x-E(x)) * (y-E(y)).T / N 
...
4. What is the v direction so the variance of the projected points will be maximum
